...
describe('AdminRolesComponent', () => {
  ...
  it('should display Collection Roles tab', async () => {
    await renderAdminRoles();
    expect(screen.getByText('Collection Roles')).toBeInTheDocument();
  });

  it('should display Exhibit Roles tab', async () => {
    await renderAdminRoles();
    expect(screen.getByText('Exhibit Roles')).toBeInTheDocument();
  });
});
