...
import {
  ...
  UserService,
} from 'src/app/generated/api/api/api';
import {
  Permission,
  User,
  UserPermission,
...
export class UserDataService implements OnDestroy {
  ...
  setLoggedInUser(authUser: AuthUser) {
    this.userService
      ...
      .subscribe((user) => {
        ...
        // merge gallery user into authUser for the topbar component.
        authUser.profile = { ...authUser.profile, ...user };
        // for authorized user, sub is a uuid of length 36
        this.isAuthorizedUser.next(
          authUser.profile.sub.length === 36
        );
        // Gallery user model doesn't have a property 'isSystemAdmin' but it's required for the topbar
        // Set it by checking permissions.
        permissions.some((p) => {
          if (p.key === 'SystemAdmin') {
            this.isSuperUser.next(true);
            this.isContentDeveloper.next(true);
            authUser.profile.isSystemAdmin = true;
          } else if (p.key === 'ContentDeveloper') {
            this.isContentDeveloper.next(true);
            authUser.profile.isContentDeveloper = true;
          }
        });
  ...
  updateUser(user: User) {
    this.userService
      .updateUser(user.id, user).pipe(take(1)).subscribe((n) => {
        this.updateStore(n);
      });
  }
